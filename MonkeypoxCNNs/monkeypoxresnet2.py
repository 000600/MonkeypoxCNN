...
history = model.fit(x_train, y_train, epochs = epochs, batch_size = batch_size, validation_data = (x_val, y_val), callbacks = [early_stopping, ModelCheckpoint("ResNet50V2-01.h5", save_best_only = True)]) # To remove early stopping, remove the "callbacks = [early_stopping]" parameter
